...
class CluStreamOnline(
  ...
  private var removeSW = true
  private var time: Long = 0L
  private var N: Long = 0L
  private var currentN: Long = 0L
  var windowTime: Int =8
  var durationStep:Long=0L
  private var microClusters: Array[MicroCluster] = Array.fill(q)(new MicroCluster(Vector(Array.fill[Double](numDimensions)(0.0)), Vector(Array.fill[Double](numDimensions)(0.0)), 0L, 0L, 0L))
  ...
  private def initRand(rdd: RDD[breeze.linalg.Vector[Double]]): Unit = {
    mcInfo = Array.fill(q)(new MicroClusterInfo(Vector(Array.fill[Double](numDimensions)(rand())), 0.0, 0L)) zip (0 until q)
  ...
  private def initKmeans(rdd: RDD[breeze.linalg.Vector[Double]]): Unit = {
    ...
    if (initArr.length >= minInitPoints) {
      ...
      mcInfo = Array.fill(q)(new MicroClusterInfo(Vector(Array.fill[Double](numDimensions)(0)), 0.0, 0L)) zip (0 until q)
        ...
        val mcs = snap1.map { mc =>
          for (mc2 <- snap2) {
            if (mc.getIds.intersect(mc2.getIds).nonEmpty) {
              ...
              if (numCount == 0) {
                mc.setCf2x(Vector(Array.fill(numDimensions)(0.0)))
                mc.setCf1x(Vector(Array.fill(numDimensions)(0.0)))
                mc.setCf2t(0L)
                mc.setCf1t(0L)
                mc.setN(0L)
              } else {
    ...
    data.foreachRDD { (rdd, timeS) =>
      ...
      if (currentN != 0) {
        ...
        /*  val t1=Instant.now()
          ...
          if(this.time==6 | this.time==8 | this.time==11 | this.time==13 | this.time==15){
          //if(this.time==6 | this.time==11 | this.time==21 | this.time==31 | this.time==41 | this.time==51 | this.time==101 |this.time==151 | this.time==201 | this.time==125){
            println(s"execution time${this.time}: ${durationStep} ms")

          }*/

          //println(s"find nearest cluster: " + durationStep  + " ms")
          var i = 0
          for (mc <- microClusters) {
            mcInfo(i) = (mcInfo(i)._1, mc.getIds(0))
            if (mc.getN > 0) mcInfo(i)._1.setCentroid(mc.cf1x / mc.n.toDouble)
         ...
         // val t1=Instant.now
          //println(Duration.between(t0, t1).toMillis+ " ms")
  ...
  def getMicroClustersInfo: Array[(MicroClusterInfo,Int)] = {
    this.mcInfo
  }
  /**
  ...
  private def updateMicroClusters(assignations: RDD[(Int, Vector[Double])]): Unit = {
    ...
    if (initialized) {
      dataInAndOut = assignations.map { a =>
        ...
        if (nearDistance <= nearMCInfo.rmsd ) { //&& tFactor * nearMCInfo.rmsd < 1
           (1, a, 0)
    ...
    timer {
      if (dataOut != null && currentN - totalIn != 0) {
       ...
       // println(s"safeDelete arr count: (${safeDeleteMC.length})")
       // println(s"keepOrMergeMC arr count: (${keepOrMergeMC.length})")
        var j = 0
