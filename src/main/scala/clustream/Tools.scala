...
object Tools {
  def main(args: Array[String]): Unit = {
    convertMCsBinariesToText("src/test/resources/snaps", "src/test/resources/snapPrint", 40)
    ...
    for (i <- 1 to limit) {
      if (Files.exists(Paths.get(dirIn + "/" + i)))
        try {
          ...
          val mc = file.readObject().asInstanceOf[ListBuffer[MicroCluster]]
          var text: ListBuffer[String] = null
          file.close()
          if (mc != null) {
            text = mc.map { m =>
              "=========================================================== \n" +
